...
export function MonthView({ currentMonth, events, onSelectDay, selectedDate }: MonthViewProps) {
    ...
    return (
        <div className="bg-slate-900/40 border border-white/5 rounded-2xl md:rounded-3xl shadow-2xl p-2 md:p-4">
            ...
            {/* Grid */}
            <div className="grid grid-cols-7 auto-rows-[80px] md:auto-rows-[120px] rounded-b-xl md:rounded-b-2xl overflow-hidden">
                {days.map((day, idx) => {
                    ...
                    return (
                        <div
                            ...
                            className={cn(
                                "min-h-0 p-2 md:p-3 border-r border-b border-white/5 transition-all cursor-pointer group hover:bg-white/[0.02]",
                        ...
                        >
                            <div className="flex justify-between items-start mb-1 md:mb-2">
                                <span
                                    className={cn(
                                        "text-[10px] md:text-xs font-bold w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-md md:rounded-lg transition-all",
                                ...
                                {dayEvents.slice(0, window.innerWidth < 768 ? 1 : 3).map((event) => {
                                    ...
                                    return (
                                        <div
                                            key={event.id}
                                            className={cn(
                                                "px-1 md:px-2 py-0.5 md:py-1 rounded-[4px] md:rounded-md text-[7px] md:text-[9px] font-bold truncate transition-all flex items-center gap-0.5 md:gap-1.5",
                                                colors.bg,
                                                colors.text,
                                        ...
                                        >
                                            <div className={cn("w-0.5 h-0.5 md:w-1 md:h-1 rounded-full shrink-0", colors.dot)} />
                                            <span className={cn(
                                                "truncate",
